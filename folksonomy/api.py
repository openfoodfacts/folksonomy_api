...
async def db_exec(response, query):
    t = time.time()
    cur.execute(query)
...
async def product_tags_list(response: Response, product: str, owner='', user: User = Depends(get_current_user)):
    if owner != '':
        if owner != user:
            return JSONResponse(status_code=422, content={"detail": {"msg": "owner should be '%s' or '' for public, was '%s'" % (user, owner)}})
...
async def product_tag(response: Response, product: str, k: str, owner='', user: User = Depends(get_current_user)):
    if owner != '':
        if owner != user:
            return JSONResponse(status_code=422, content={"detail": {"msg": "owner should be '%s' or '' for public, was '%s'" % (user, owner)}})
